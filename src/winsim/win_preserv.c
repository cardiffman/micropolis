...
typedef struct {
    ...
    Win_FreeProc *freeProc;	/* Procedure to call to free. */
} Reference;
    ...
    ClientData clientData;	/* Pointer to malloc'ed block of memory. */
{
    ...
    if (inUse == spaceAvl) {
	if (spaceAvl == 0) {
	    refArray = malloc(INITIAL_SIZE*sizeof(Reference));
	    spaceAvl = INITIAL_SIZE;
	} else {
	    Reference *new;

	    new = realloc(refArray, 2*spaceAvl*sizeof(Reference));
	    refArray = new;
	    spaceAvl *= 2;
	}
    }
    ...
    ClientData clientData;	/* Pointer to malloc'ed block of memory. */
{
	...
	if (refPtr->refCount == 0) {
	    if (refPtr->mustFree) {
		if (refPtr->freeProc == (Win_FreeProc *) free) {
		    free(refPtr->clientData);
		} else {
	    ...
	    inUse--;
	    if (i != inUse) {
		memcpy(&refArray[i], &refArray[i+1],
