...
int TryOther(int Tpoo, int Told, int Tnew);
...
#define SPRITECMD_ACCESS_INT(var) \
  int SpriteCmd##var(SimSprite *sprite, Tcl_Interp *interp, int argc, char **argv) { \
    ...
    Tcl_SetObjResult(interp, Tcl_NewIntObj(sprite->var)); \
    return (TCL_OK); \
  }
...
#define SPRITECMD_GET_STR(var) \
  int SpriteCmd##var(SimSprite *sprite, Tcl_Interp *interp, int argc, char **argv) { \
    Tcl_SetResult(interp, sprite->var, TCL_STATIC); \
    return (TCL_OK); \
  }


int
DoSpriteCmd(ClientData clientData, Tcl_Interp *interp, int argc, const char **argv)
{
...
int
SpriteCmd(ClientData clientData, Tcl_Interp *interp, int argc, const char **argv)
{
  SimSprite *sprite;
  int type;
  ...
  Tcl_SetResult(interp, sprite->name, TCL_STATIC);
  return TCL_OK;
}
...
SimSprite *
NewSprite(const char *name, int type, int x, int y)
{
  SimSprite *sprite;
